...
若某路由会拒绝不携带会话身份的请求，该请求会在其端点要求的标头中带上 Harness 会话 id。已安装目录的 OpenCode Go 路由在缺少 `x-opencode-session` 时以 `400 MissingSessionID` 作答，因此 `dsh-llm-pi-ai` 会在每条被 agent loop 打上 Session id 的请求上发送该标头；不携带会话 id 的请求不发送该标头，而不会编造一个值。标头名与模型目录同属已安装提供方的固有属性，而不是 profile 的字段：端点要么要求该标头，要么不要求，pi-ai 的目录并未为它声明任何标头。
...
**把要求的会话标头做成配置字段。** 增加 `sessionHeader` 字段后，指向同一网关的手工声明路由也能带上目录路由默认获得的要求。但它同时让一条可用路由取决于运维者是否知道一个已安装提供方本就蕴含的标头名，而当前没有任何部署声明这类路由；因此该要求随目录事实存放，等到确实有手工声明路由需要时再增加配置字段。
...
- 要求会话标头的目录路由会在每条被 agent loop 打上 Session id 的请求上携带该标头，因此指向这类路由的无会话直接 `ctx.llm.stream()` 调用会被提供方拒绝，而不会以编造的身份被路由。
...
- 单元测试覆盖注册表冲突、请求重建、会话验证、配置解析、单次请求的选项转发、路由要求的会话标头、包括 OpenAI Responses 在内的原生 API 选择、转换、回放验证、错误映射、调用方取消、空闲超时导致的传输终止、内容重写，以及同一实例与不同实例间的回放分发。
