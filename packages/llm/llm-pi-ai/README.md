...
One route serves a model the installed catalog has not caught up with through a harness-owned bridge: the model is cloned from a named sibling entry of the same route, so the wire protocol, compat switches, thinking map, and capacities ride along, and only the id and display name change. The OpenCode Go route offers `deepseek-v4.1-flash` (DeepSeek V4.1 Flash, 1M context / 384K output) this way until the installed catalog ships it; a bridged id the catalog later ships keeps the catalog's own entry, and the bridge entry should then be removed.
